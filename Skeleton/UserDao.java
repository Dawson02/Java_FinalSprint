...
public class UserDao {
    public boolean createUser(User user) {
        ...
        String query = "INSERT INFO users ( first_name, last_name, email, password, is_doctor, doctor_id ) " + "VALUES ( ?, ?, ?, ?, ? )";
        ...
        try {
            ...
            statement.setInt(6, user.getDoctorId());
    ...
    public User getUserById(int id) { //get user by id from database 
        ...
        return new User(user_id, firstName, lastName, email, password, is_doctor, doctor_id);
    ...
    public User getUserByEmail(String email) { // get user by email from database 
        ...
        return new User(id, firstName, lastName, user_email, password, is_doctor, doctor_id);
    }

    public boolean updateUser(User user) { // update user in the database
        boolean bool = false;

        // Prepare the SQL query
        String query = "UPDATE users " +
                "SET first_name = ?, last_name = ?, email = ?, password = ?, is_doctor = ?, doctor_id = ? " + "WHERE id = ?";
        String hashedPassword = BCrypt.hashpw(user.getPassword(), BCrypt.gensalt());
        ...
        try { 
            ...
            statement.setInt(7, user.getDoctorId());
            int updatedRows = statement.executeUpdate();
            if (updatedRows != 0) {
                bool = true;
            }
